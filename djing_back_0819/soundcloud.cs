...
namespace DJing
{
    public partial class soundcloud : Form
    {
        ...
        {
            btnMyPage.Left = topPanel.ClientSize.Width - btnMyPage.Width - 10;
            CenterSearchControls();
        }
        ...
        // 로그아웃 과정에서 창을 닫아도 프로그램이 종료되지 않도록 설정
        ...
        {
            // 로그인 창에서 바로 열린 게스트/관리자 화면을 X로 닫으면 프로그램 종료
            if (!isNavigating && Owner == null)
            {
                Application.Exit();
            }
        }
