...
namespace WinFormsApp_0812
{
    public partial class Form1 : Form
    {
        ...
        {
            // trackBar1 범위 및 초기값 설정 (-12 ~ +12 반음)
            trackBar1.Minimum = -12; // 1옥타브 DOWN
            trackBar1.Maximum = 12;  // 1옥타브 UP
            trackBar1.Value = 0;     // 원본 피치 (0)
        ...
        // trackBar1 슬라이더 조절 이벤트
        private void trackBar1_Scroll(object sender, EventArgs e)
        {
            int semitones = trackBar1.Value; // -12 ~ +12 값 추출

            // 디버그 출력
            System.Diagnostics.Debug.WriteLine($"[Pitch 변경] {semitones} Semitones");

            // 피치 조절 엔진으로 반음 값 전달
            eqEngine.SetPitch(semitones);
        }

        // 피치 초기화 버튼 (필요 시 연동)
        private void btnResetPitch_Click(object sender, EventArgs e)
        {
            trackBar1.Value = 0;
            eqEngine.SetPitch(0);
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            using (OpenFileDialog ofd = new OpenFileDialog { Filter = "Audio Files|*.mp3;*.wav" })
            {
                if (ofd.ShowDialog() == DialogResult.OK)
                {
                    ...
                    // 재생 전 현재 트랙바 피치 값 먼저 적용
                    eqEngine.SetPitch(trackBar1.Value);
                    eqEngine.Play();
                }
            }
        }
