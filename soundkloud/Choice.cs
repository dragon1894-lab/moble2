...
namespace DJing
{
    public partial class Choice : Form
    {
        ...
        {
            ProfileEdit profileEdit = new ProfileEdit(UserSession.UserId);
            ...
            if (profileEdit.ShowDialog() == DialogResult.OK)
            {
                // ① 클릭한 버튼의 이미지를 PictureBox에 즉시 세팅
                pb_Profile.Image = profileEdit.SelectedImage;

                // ② DB의 profile_icon 컬럼에 "icon1", "icon2" 등 선택된 이름 UPDATE
                UpdateProfileIconToDB(profileEdit.SelectedIconName);
            }
        }
