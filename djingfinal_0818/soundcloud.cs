...
namespace DJing
{
    public partial class soundcloud : Form
    {
        ...
        public soundcloud()
        {
            ...
            InitPlayerTimer();
            LoadSongsToUI();
            this.FormClosed += soundcloud_FormClosed;
        }
        private void soundcloud_FormClosed(object sender, FormClosedEventArgs e)
        {
        ...
        private void BtnMyPage_Click(object sender, EventArgs e)
        {
            ...
            myPage.Location = this.Location; // Form1 위치 전달

            this.Hide();
            myPage.ShowDialog();

            // Mypage 닫힌 후 처리
            this.Location = myPage.Location;

            if (bottomPlayerPanel != null)
            {
                this.Controls.Add(bottomPlayerPanel);
        ...
        public void LoadSongsToUI(string keyword = "")
        {
            ...
            for (int i = 0; i < currentSongList.Count; i++)
            {
                ...
                {
                    if (currentSongIndex == index && globalWaveOut != null)
                    {
                        BtnBottomPlay_Click(s, EventArgs.Empty);
                    }
                    else
                    {
        ...
        {
            for (int i = 0; i < songListPanel.Controls.Count; i++)
            {
                if (songListPanel.Controls[i] is SongItemControl item)
                {
                    bool isThisPlaying = (i == currentSongIndex) && (globalWaveOut?.PlaybackState == PlaybackState.Playing);
                    double progress = 0;
                    if (i == currentSongIndex && globalAudioFile != null && globalAudioFile.Length > 0)
                    {
                        progress = (double)globalAudioFile.Position / globalAudioFile.Length;
                    }
                    item.UpdateWaveState(isThisPlaying, progress);
                }
            }
        }
