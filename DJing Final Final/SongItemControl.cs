...
namespace DJing
{
    internal class SongItemControl : UserControl
    {
        ...
        private void BtnComment_Click(object? sender, EventArgs e)
        {
            ...
            if (ownerForm != null)
            {
                commentForm.StartPosition = FormStartPosition.Manual;

                commentForm.Location = new Point(
                    ownerForm.Left +
                    (ownerForm.Width - commentForm.Width) / 2,

                    ownerForm.Top +
                    (ownerForm.Height - commentForm.Height) / 2
                );

                commentForm.Show(ownerForm);
            }
            else
            {
                commentForm.StartPosition =
